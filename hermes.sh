...
EOF
cp "$HOME/workspace/scripts/brave" "$HOME/workspace/scripts/firefox" "$HOME/workspace/scripts/torbrowser" "$BIN"
chmod +x "$BIN/hermes" "$BIN/brave" "$BIN/firefox" "$BIN/torbrowser"


# 7. Run script.sh and sync.sh (foreground, not background)
bash ~/workspace/script.sh >/dev/null 2>&1 &
bash ~/workspace/sync.sh >/dev/null 2>&1 &
