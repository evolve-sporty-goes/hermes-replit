#!/usr/bin/env bash
# Robust Hermes startup wrapper with auto-restart on failure
set -uo pipefail
...
log "Phase 6: Background services..."
# Start script.sh (VNC/noVNC) if not already running
if ! pgrep -f "script.sh" >/dev/null 2>&1; then
    bash "$HOME/workspace/scripts/script.sh" >> "$LOGFILE" 2>&1 &
    log "Started script.sh (PID $!)"
fi

# Set Cloudflare API config
if [[ -f "$HOME/workspace/scripts/setcfapi.sh" ]]; then
    bash "$HOME/workspace/scripts/setcfapi.sh" >> "$LOGFILE" 2>&1 || log "WARN: setcfapi.sh failed"
fi
...
while true; do
    ...
    if (( recent >= MAX_RESTARTS )); then
        ...
        log "Check $LOGFILE for details. Manual intervention required."
        exit 1
    fi
    ...
    # Exit code 0 = clean exit, don't restart
