...
cat > ~/or_proton.py << 'PY'
import sys, re, time, html  # <-- Added html import here
from cloakbrowser import launch_persistent_context
...
def find_verify():
    ...
    for frame in page.frames:
        try:
            ...
            # Optional: print(f"Frame read error: {e}") if you need to debug
            pass           

    # Fallback: Check elements in the main frame context
    try:
...
for attempt in range(15):
    ...
    for i in range(min(count, 5)):
        ...
        if subj not in checked:
            checked.add(subj)
            items.nth(i).click()
            page.wait_for_timeout(5000) # Give the iframe extra time to load content
