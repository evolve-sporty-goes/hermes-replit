#!/usr/bin/env bash
# torbox-camoufox-signup.sh — TorBox full signup via Camoufox + rotating free SOCKS5
#
# Strategy:
# 1) Grab free SOCKS5 proxies from ProxyScrape
# 2) Test each proxy + Camoufox (geoip=True) against torbox.app
# 3) First proxy that passes CF wins → full signup pipeline through it
# 4) Fallback: Supabase API signup via Tor+FlareSolverr, then Camoufox for dashboard
#
# Requires: camoufox, playwright, PySocks, tor, flaresolverr
set -euo pipefail
...
echo "========================================"
echo " TorBox Signup via Camoufox + Free SOCKS5"
echo "========================================"
echo "Email: $EMAIL"
...
#         via Camoufox + free SOCKS5 (rotating)
# ═══════════════════════════════════════════
echo ""
echo "→ [3/4] Verify + Login + Demo + API Key via Camoufox ..."
...
def wait_for_cf(page, timeout=15):
    """Wait for Cloudflare to auto-solve in Camoufox."""
...
for proxy_idx, proxy_addr in enumerate(proxies[:MAX_PROXY_TRIES]):
    ...
    td = tempfile.mkdtemp(prefix="torbox-camoufox-")
