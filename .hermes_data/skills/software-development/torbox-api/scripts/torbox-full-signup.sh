...
#   Step 3: visit verify URL + extract API key via Playwright + Tor
set -euo pipefail
...
PASSWORD="${2:...shuf -i 10000-99999 -n 1)
...
with open("/tmp/tb_verify_email.txt") as f: email = f.read().strip()
...
API_KEY=*** "$STEP3_OUT" | grep "^API_KEY=*** | cut -d= -f2-)
