...
**Verification reporting style:** When reporting verification results, be terse. State what passed/failed and the file:line evidence. Do not explain why `&&` short-circuits or why a check exists — the user already knows. Example: "PASS: push guard at install.sh:37 — `git push && last=...` nested under `if commit`." Not: "The `&&` operator means that if push fails, the right side won't execute, which is important because..."
...
| Cloudflare Workers AI | API key + Account ID | `CLOUDFLARE_API_KEY` + `CLOUDFLARE_ACCOUNT_ID` (see `references/cloudflare-provider.md`) |
| Fallback chain | `fallback_providers` | List of `{provider, model, base_url?, api_mode?}` dicts (see `references/fallback-providers.md`) |
| Custom endpoint | Config | `model.base_url` + `model.api_key` in config.yaml or legacy `cloudflare:` section |
| GitHub Copilot ACP | External | `COPILOT_CLI_PATH` or Copilot CLI |

Full provider docs: https://hermes-agent.nousresearch.com/docs/integrations/providers
Cloudflare-specific setup and pitfalls: `skill_view(name="hermes-agent", file_path="references/cloudflare-provider.md")`

### Provider Connectivity on Restricted Networks

Some sandbox environments (notably Replit Nix) have DNS whitelists that block
provider subdomains (e.g. `api-inference.huggingface.co` fails to resolve while
`huggingface.co` works). If a provider fails with `APIConnectionError` before any
HTTP exchange, check DNS first — see `references/provider-dns-restrictions.md`.
...
**Template: auto-push-watcher.sh** — see `templates/auto-push-watcher.sh` for the canonical resilient version. Copy it into `install.sh`, adjusting the `cd` path.

**Common pattern — auto-commit/push during pairing sessions:**

When a human is pairing with an agent on a long task, keep work committed and pushed. Two approaches:

1. **Cron job** (periodic, good for "back up every few minutes"):
```python
cronjob(action="create", schedule="every 1m", name="auto-commit-push",
  prompt="Check git status in /home/runner/workspace. If there are changes, commit them with a descriptive message and push to origin. Skip nothing — commit all files including data dirs. If nothing to commit, say so briefly.")
```

2. **File watcher** (reactive, good for "push on every change"): Run a background Python script that polls the filesystem, debounces, and auto-commits/pushes. For a resilient bash implementation (correct cooldown-on-failure, SIGTERM trap, debounce), see `references/bash-watchdog-resilience.md`.

Choose cron when the user wants a safety net on a schedule. Choose the watcher when the user explicitly wants "push for every change" or finds periodic checks too slow.
...
5. **Intermittent 5xx / rate limits**: Configure `fallback_providers` so Hermes can retry with a backup model automatically. See `references/fallback-providers.md`.
