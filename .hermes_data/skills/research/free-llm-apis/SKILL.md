...
| **Best for coding** | **Cloudflare (Kimi K2.7-code), OpenRouter (DeepSeek R1 / Qwen3 Coder)** |
...
After configuring a model, confirm it works — but **do not read .env files or secrets files from test scripts**. Instead:

1. Use `hermes config` to confirm the model and provider are set correctly.
2. Use `hermes doctor` to check provider connectivity.
3. Optionally run a one-shot query: `hermes chat -q "Say hello" -m MODEL_NAME` — this uses the key from .env without exposing it in your script.

**Pitfall:** Never write a Python/shell script that reads `~/.hermes/.env` and passes the key to an API call. This combines secret-file access with network calls in one command, which users may block. Prefer `hermes doctor` or a `hermes chat -q` one-shot probe instead.
...
- When setting a free model on OpenRouter, append `:free` to the model ID (e.g. `nvidia/nemotron-3-ultra-550b-a55b:free`). Without the suffix, OpenRouter routes to the paid variant.
